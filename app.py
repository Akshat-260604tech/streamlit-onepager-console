...
Deployed version for Streamlit Cloud
"""
...
def render_additional_analytics(records: List[OnePagerRecord]):
    """Render additional analytics and insights"""
    ...
    # Create two columns for different analytics
    ...
    with col1:
        ...
        # Excel usage analysis
        st.subheader("📊 Excel File Usage")
        excel_provided = len([r for r in records if r.excel_provided])
        no_excel = len(records) - excel_provided

        excel_data = {
            'Type': ['With Excel', 'Without Excel'],
            'Count': [excel_provided, no_excel]
        }

        fig_excel = px.pie(
            values=excel_data['Count'],
            names=excel_data['Type'],
            title="Excel File Usage",
            color_discrete_map={
                'With Excel': '#28a745',
                'Without Excel': '#6c757d'
            }
        )
    ...
    with col2:
        ...
        st.subheader("☁️ Azure Upload Success")

        azure_success = len([r for r in records if r.azure_upload_ok])
        azure_failed = len(records) - azure_success

        if azure_success + azure_failed > 0:
            success_rate = (azure_success / (azure_success + azure_failed)) * 100

            col2a, col2b = st.columns(2)
            with col2a:
                st.metric("Success Rate", f"{success_rate:.1f}%")
            with col2b:
                st.metric("Total Uploads", azure_success + azure_failed)

            # Upload success pie chart
            upload_data = {
                'Status': ['Successful', 'Failed'],
                'Count': [azure_success, azure_failed]
            }

            fig_upload = px.pie(
                values=upload_data['Count'],
                names=upload_data['Status'],
                title="Azure Upload Status",
                color_discrete_map={
                    'Successful': '#28a745',
                    'Failed': '#dc3545'
                }
            ...
            st.info("No Azure upload data available")
