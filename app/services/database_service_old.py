...
class DatabaseService:
    ...
    def _extract_excel_blob_info(self, record_data: dict) -> dict:
        """Helper method to extract Excel blob info from database record"""
        if record_data.get('excel_blob_info') and isinstance(record_data['excel_blob_info'], dict):
            record_data['excel_blob_url'] = record_data['excel_blob_info'].get('excel_blob_url')
            record_data['excel_blob_path'] = record_data['excel_blob_info'].get('excel_blob_path')
        else:
    ...
    async def create_one_pager_record(self, record_data: OnePagerRecord) -> Optional[OnePagerRecord]:
        ...
        try:
            ...
            if result.data and len(result.data) > 0:
                # Extract Excel blob fields using helper method
                created_data = self._extract_excel_blob_info(result.data[0])
